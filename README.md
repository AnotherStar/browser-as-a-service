...
Для локальной диагностики можно принудительно отключить прокси на стороне
сервиса: `IGNORE_PROXY=1` в `.env`. Тогда сервис идёт напрямую и игнорирует и
переданный в `/run` `proxy`, и настроенный `ASOCKS_API_KEY`; создаваемые
`/sessions` тоже будут direct.
...
**Тип прокси на сессию (`proxy_type`).** Некоторым сайтам нужен конкретный тип
exit-IP: Yandex.Market метит капчей все IP, кроме реального мобильного 4G.
Поэтому `POST /sessions` принимает `proxy_type` (`residential`/`mixed`/`mobile`/
`corporate`); пусто — серверный дефолт (`ASOCKS_PROXY_TYPE_ID`, обычно `mixed`).
Сервис сам создаёт порт нужного типа (`proxy/create-port` с соответствующим
`proxy_type_id`) и переиспользует его: авто-созданные типизированные порты
помечаются именем `baas-pt<id>`, по нему они и подбираются повторно (если листинг
не отдаёт тип явно). Подбор и кэш портов раздельны по (страна, тип), так что
запрос `mobile` никогда не получит mixed-IP.
